function Portal(data) {
  ...
  // Profile
  this.profile_wr = document.createElement("div");
  this.profile_wr.id = "profile";
  this.icon_el = document.createElement("div");
  this.icon_el.className = "icon";
  this.name_el = document.createElement("t");
  this.name_el.className = "name";
  this.desc_el = document.createElement("t");
  this.desc_el.className = "desc";
  this.site_el = document.createElement("t");
  this.site_el.className = "site";
  ...
  // Activity
  this.activity_wr = document.createElement("div");
  this.activity_wr.id = "activity";
  this.entries_el = document.createElement("t");
  this.entries_el.className = "entries";
  this.portals_el = document.createElement("t");
  this.portals_el.className = "portals";
  ...
  this.port_status_el = document.createElement("t");
  this.port_status_el.className = "port_status";

  this.port_list_el = document.createElement("t");
  this.port_list_el.className = "port_list";
  ...
  this.update = function() {
    ...
    this.site_el.innerHTML =
      "<a href='" +
      this.data.site +
      "' target='_blank'>" +
      this.data.site.replace(/^(https?:|)\/\//, "") +
      "</a>";
    this.desc_el.innerHTML = this.data.desc;

    this.entries_el.innerHTML =
      this.data.feed.length +
      " <unit>" +
      (this.data.feed.length == 1 ? "Entry" : "Entries") +
      "</unit>";
    this.portals_el.innerHTML = this.data.port.length + "<unit>Portals</unit>";
    this.portals_el.className =
      this.data.port.length > 45 ? "portals limit" : "portals";
