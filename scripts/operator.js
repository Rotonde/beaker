function Operator() {
  ...
  this.input_el.id = "commander";
  this.input_el.setAttribute("placeholder", "Input command here");
  this.hint_el = document.createElement("t");
  this.hint_el.id = "hint";
  ...
  this.update = function() {
    ...
    var key = this.input_el.value.split(" ")[
      this.input_el.value.split(" ").length - 1
    ];
    var autocomplete = key ? this.find_portal_with_key(key) : null;

    if (
      (key.substr(0, 1) == "@" || key.substr(0, 1) == "~") &&
      autocomplete &&
      autocomplete != "@" + key &&
      autocomplete != "~" + key
    ) {
  ...
  this.validate = function() {
    var command = this.input_el.value.indexOf(" ")
      ? this.input_el.value.split(" ")[0]
      : this.input_el.value;
    var params = this.input_el.value.indexOf(" ")
      ? this.input_el.value
          .split(" ")
          .slice(1)
          .join(" ")
    ...
    if (e.key == "Tab") {
      ...
      if (name_match) {
        for (var portal_name in r.feed.portals) {
          if (
            portal_name &&
            portal_name.substr(0, name_match[1].length) === name_match[1]
          ) {
      ...
      if (
        ...
        reader.onload = async function(e) {
          ...
          if (r.operator.input_el.value) {
            commanderText =
              r.operator.input_el.value.trim() + " >> " + file.name;
          }
